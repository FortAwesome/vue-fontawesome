...
Here's a
[CodeSandbox Starter Sample](https://codesandbox.io/s/github/FortAwesome/vue-fontawesome/tree/master/examples/vue-cli-webpack)
on how to display Solid, Regular, and Brand icons
[using the Icon Library](https://github.com/FortAwesome/vue-fontawesome#usage).
...
**Visit [fontawesome.com/icons](https://fontawesome.com/icons) to search for
free and Pro icons**
...
If you are a [Font Awesome Pro](https://fontawesome.com/pro) subscriber you can
install Pro packages after these
[additional configuration](https://fontawesome.com/how-to-use/on-the-web/setup/using-package-managers)
are made.
...
The following examples are based on a project configured with
...
Using the Pro packages requires
[additional configuration](https://fontawesome.com/how-to-use/on-the-web/setup/using-package-managers).
...
You can also
[import the same icon from different styles](#import-the-same-icon-from-different-styles)
with some help from ES `import`.

#### Quick warning about self-closing tags

If you are using inline templates or HTML as templates you need to be careful to
...
A basic installation of
[Font Awesome](https://fontawesome.com/how-to-use/on-the-web/setup/getting-started?using=svg-with-js)
has the ability to automatically transform `<i class="fas fa-coffee"></i>` into
...
The `icon` property of the `FontAwesomeIcon` component can be used in the
following way:
...
For the above to work you must add the `spinner` and `align-left` icon to the
library.
...
you would need to pass in the icon name using _kebab-case_ as opposed to
_camelCase_.
...
For the above to work you must add the regular `spinner` icon (Pro only) to the
library.
...
Explicitly selecting icons offer the advantage of only bundling the icons that
you use in your final bundled file. This allows us to subset Font Awesome's
...
large. We **highly** recommend that you take advantage of subsetting through
tree shaking.
...
If you are not using a tool that supports tree shaking **you may end up bundling
more icons than you intend**. Here are some alternative import syntaxes:
...
The following features are available as part of Font Awesome. Note that the
syntax is different from our general web-use documentation.

### Register your components first

To use the following examples you must first register your component so Vue is
aware of it.

A good place to do this is in `main.js` or in the module you are calling
`new Vue()`. **Make sure you register the component** and **have added icons to
your library** before you bootstrap your Vue application.
...
Spin and pulse
[animation](https://fontawesome.com/how-to-use/on-the-web/styling/animating-icons):
...
Install `@fortawesome/vue-fontawesome` and `@fortawesome/fontawesome-svg-core`
and any icon packages.
...
If you use PurgeCSS, or use the nuxt.js tailwindcss module which comes with
PurgeCSS prebundled, you need to add fontawesome css classes to the whitelist,
as the classes only gets inserted on rendering, and PurgeCSS will treat them as
unused and remove them otherwise.

In your `nuxt.config.js`, add a purgeCSS config object. You may adjust the regex
to your liking:
...
The Vue
[project provides a wrapper](https://github.com/vuejs/vue-web-component-wrapper)
that will register your Vue components as
...
Here is an example that leverages the `mounted()` lifecycle hook to insert the
CSS.
...
icon from the library is a bit cumbersome. Initially, this does seem to be the
case but we do have good reasons.
...
The problem with this is that it does not provide a consistent or concise way to
specify the mask.
...
Does the `far` apply to the icon or the mask? What is the prefix for the
property it does not apply to?
...
If we allow prefix definition through a property and we also allow an icon to be
specified as an object through direct import these two have a chance to argue
with eachother. This could lead to some head-scratching when an icon doesn't
appear in the expected style.

In the following case which style should be used (light from the icon definition
or regular from the far boolean):
...
The following contributors have either hepled to start this project, have
...
If we've missed someone (which is quite likely) submit a Pull Request to us and
